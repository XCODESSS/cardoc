...
- [ ] Write tests first for save/get after a new cache instance (simulated process restart), missing/deleted files, remove, same document ID under different user IDs, partial-copy failure, and path traversal strings rejected for both IDs. Run `npm test -- --runInBand tests/document-cache.test.ts`; expect failure before implementation.
- [ ] Use current Expo `Directory`, `File`, and `Paths.document` APIs under an account-scoped `cardoc/<userId>/` directory. Validate `userId` and `documentId` as UUIDs before constructing paths. Require `sourceUri` to point to a staged `.pdf`, `.jpg`, `.jpeg`, or `.png` file; Task 5 stages picker/download bytes under the extension validated from MIME. Copy to a temporary file, verify nonzero size, then move to the final name; `get` must check the physical file on every call. Do not store a source picker URI as the cache pointer.
